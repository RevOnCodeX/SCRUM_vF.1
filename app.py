...
def _nokia_paste_trailing_roster_name_hints(text: str, roster: Sequence[str]) -> set[str]:
    ...
    detect roster names that appear as a line suffix (Nokia rows often end with the employee name).
    """
...
def _nokia_paste_employee_must_match_selected_or_error(
    selected: str, text: str, roster: Sequence[str]
) -> str | None:
    """
    If pasted Nokia lines clearly name one or more roster employees (suffix heuristic), ensure they
    match the dropdown selection. Returns an error message to show, or None when OK / not verifiable.
    """
...
def build_leave_tracker_month_xlsx_bytes(month_ctx: dict[str, Any]) -> tuple[bytes | None, str | None]:
    ...
    a1 = ws.cell(row=1, column=1, value=month_name)
    a1.fill = hdr_fill
    a1.font = hdr_font
    a1.alignment = center
    ...
    b1 = ws.cell(row=1, column=2, value="Total")
    b1.fill = hdr_fill
    b1.font = hdr_font
    b1.alignment = center
    ...
    c1 = ws.cell(row=1, column=3, value="eLeaveCount")
    c1.fill = hdr_fill
    c1.font = hdr_font
    c1.alignment = center
    ...
    d1 = ws.cell(row=1, column=4, value="GAP")
    d1.fill = hdr_fill
    d1.font = hdr_font
    d1.alignment = center
    ...
    for grow in grid_rows:
        n1 = ws.cell(row=r, column=1, value=str(grow.get("employee") or ""))
        n1.font = body_font
        ...
        try:
            ...
            tot_v = 0.0
        n2 = ws.cell(row=r, column=2, value=tot_v)
        n2.font = body_font
        n2.alignment = center
        ...
        try:
            ...
            gap_v = 0.0
        n3 = ws.cell(row=r, column=3, value=el_v)
        n3.font = body_font
        n3.alignment = center
        n3.border = grid_border
        n4 = ws.cell(row=r, column=4, value=gap_v)
...
def _scrum_attachment_upload_core(
    ...
    *,
    ...
    if _sprint_is_closed_by_id(conn, sprint_id):
        return False, SCRUM_SPRINT_READONLY_FLASH, ""
    row = conn.execute(
...
def _enrich_kanban_burn_cards(conn: sqlite3.Connection, kanban_cards: list[dict]) -> None:
    ...
    for card in kanban_cards:
        ...
        if est > SCRUM_HOUR_EPS:
            ...
            burn_pct = None
        card["committed_logged_hours"] = round(committed, 2)
        card["burn_pct"] = burn_pct
        card["effort_scale_max"] = round(mx, 2)
        card["pct_est_bar"] = pct_est
        card["pct_committed_regular_bar"] = pct_committed_regular
        card["pct_stretch_bar"] = pct_stretch
        card["stretch_load_hours"] = round(stretch, 2)
...
def create_app() -> Flask:
    ...
    def scrum_sprint_team(sprint_id: int):
        ...
        if sprint_team_capacity_hours is not None and not _sprint_row_is_closed(sprint):
            ts = _utc_stamp()
            conn.execute(
                """
                UPDATE scrum_sprint
                ...
                if audit_action == "show_approved":
                    if not employee_name or employee_name not in g.manager_roster:
                        ...
                        name_err = _nokia_paste_employee_must_match_selected_or_error(
                            employee_name, csv_text_input, g.manager_roster
                        )
                        if name_err:
