...
def test_dashboard_leave_tracker_renders(client):
    ...
    ws = wb["July 2026"]
    assert ws.cell(row=1, column=1).value == "July"
    assert ws.cell(row=1, column=2).value == "Total"
    assert ws.cell(row=1, column=3).value == "eLeaveCount"
    assert ws.cell(row=1, column=4).value == "GAP"
    assert ws.cell(row=1, column=5).value == "Wed"
    sat_cols = [col for col in range(5, 5 + 31) if ws.cell(1, col).value == "Sat"]
    sun_cols = [col for col in range(5, 5 + 31) if ws.cell(1, col).value == "Sun"]
...
def test_nokia_audit_show_approved_includes_all_dates_not_only_form_month(client):
    ...
    name = EMPLOYEES[0]
    txt = (
        "Leave Entry 1 Approved 29-05-2026 29-05-2026\n"
        "Leave Entry 2 Approved 31-07-2026 31-07-2026\n"
    )
    r = client.post(
        "/worksheet/nokia-audit",
        data={
...
def test_nokia_mark_appends_marker_when_day_already_on_tracker(client, app):
    ...
    conn.commit()
    conn.close()
    txt = "Leave Entry 999 Approved 20-05-2026 20-05-2026\n"
    assert (
        client.post(
            "/worksheet/nokia-audit",
            data={
